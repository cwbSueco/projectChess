...
//		check = (testCheck(opponent(currentPlayer))) ? true : false;
	...
	public ChessPiece replacePromotedPiece(String type) {
		...
		if (!type.equals("Q") && !type.equals("B") && !type.equals("H") && !type.equals("R")) {
			return promoted;
		}
...
//			rook.decreaseMoveCount();
	...
	private void validateTargetPosition(Position source, Position target) {
		if (!board.piece(source).possibleMove(target)) {
			throw new ChessException("Target position is not a valid move");
		}
	}
	...
	private boolean testCheck(Color color) {
		...
		List<Piece> enemies = piecesOnTheBoard.stream().filter(x -> ((ChessPiece) x).getColor() == opponent(color))
				.collect(Collectors.toList());
		for (Piece p : enemies) {
...
//			}
	...
	private boolean testCheckMate(Color color) {
		...
		List<Piece> friends = piecesOnTheBoard.stream().filter(x -> ((ChessPiece) x).getColor() == color)
				.collect(Collectors.toList());
		for (Piece p : friends) {
			boolean[][] mat = p.possibleMoves();
			for (int i = 0; i < board.getRows(); i++) {
				for (int j = 0; j < board.getColumns(); j++) {
					if (mat[i][j]) {
						...
						Piece cp = makeMove(source, target);
						boolean check = testCheck(color);
						undoMove(source, target, cp);
						if (!check) {
							return false;
						}
					}
